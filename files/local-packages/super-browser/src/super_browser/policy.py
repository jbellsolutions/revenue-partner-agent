...
EXTERNAL_WRITE_PATTERNS = (
    r"\b(?:create|run|launch|start|activate|enable|restart|resume)\b[^.!?;\n]{0,160}\b(?:ads?|adverts?|advertisements?|advertising|campaigns?)\b",
    r"\b(?:turn|switch)\s+on\b[^.!?;\n]{0,160}\b(?:ads?|adverts?|advertisements?|advertising|campaigns?)\b",
    r"\b(?:take|put|set)\b[^.!?;\n]{0,120}\b(?:ads?|adverts?|advertisements?|advertising|campaigns?)\b[^.!?;\n]{0,80}\b(?:live|active)\b",
...
AD_CAMPAIGN_OBJECT_PATTERNS = (
    r"\b(?:ads?|adverts?|advertisements?|advertising|campaigns?)\b",
)
EXPLICIT_AD_CAMPAIGN_READ_ONLY_PATTERNS = (
    r"^\s*(?:(?:please|kindly)\s+)?(?:(?:can|could|would) you\s+)?(?:analyze|analyse|audit|compare|evaluate|inspect|read|research|search|look up|review|summarize|summarise)\b[^.!?;\n]{0,200}\b(?:ads?|adverts?|advertisements?|advertising|campaigns?)\b(?:\s+(?:performance|results|metrics|examples|history|copy|creative|structure|strategy|reporting|analytics|data|documentation)){0,3}\s*(?:please)?[?.]?\s*$",
    r"^\s*(?:(?:please|kindly)\s+)?(?:(?:can|could|would) you\s+)?(?:explain|describe|define)\b[^.!?;\n]{0,200}\b(?:ads?|adverts?|advertisements?|advertising|campaigns?)\b(?:\s+(?:performance|results|metrics|examples|history|copy|creative|structure|strategy|reporting|analytics|data|documentation)){0,3}\s*(?:please)?[?.]?\s*$",
    r"^\s*(?:what|which|how)\b[^.!?;\n]{0,200}\b(?:ads?|adverts?|advertisements?|advertising|campaigns?)\b(?:\s+(?:performance|results|metrics|examples|history|copy|creative|structure|strategy|reporting|analytics|data|documentation)){0,3}\s*[?.]?\s*$",
    r"^\s*(?:find|list|show|give|send)\s+(?:me\s+|us\s+)?(?:examples|a report|the report|a summary|the summary|information|details|metrics|results|data)\b[^.!?;\n]{0,160}\b(?:ads?|adverts?|advertisements?|advertising|campaigns?)\b\s*[?.]?\s*$",
)
EXPLICIT_AD_CAMPAIGN_PUBLIC_SEARCH_PATTERNS = (
    r"^\s*submit\s+(?:the\s+)?public\s+search\s+(?:form|query|box|field)\s+for\s+(?:visible\s+)?(?:ads?|adverts?|advertisements?|advertising|campaigns?)\s+(?:examples|results)\s*[?.]?\s*$",
    r"^\s*press\s+(?:enter|return)\s+to\s+submit\s+(?:the\s+)?public\s+search\s+(?:form|query|box|field)\s+for\s+(?:visible\s+)?(?:ads?|adverts?|advertisements?|advertising|campaigns?)\s+(?:examples|results)\s*[?.]?\s*$",
)
AD_CAMPAIGN_PRODUCTION_ACTION_PATTERNS = (
    r"\b(?:create|run|launch|start|activate|enable|restart|resume|schedule|promote|deploy|execute|publish|post|submit|upload)\b",
    r"\b(?:kick\s+off|fire\s+up|go\s+live|roll\s+out|turn\s+on|switch\s+on)\b",
    r"\b(?:take|put|set)\b[^.!?;\n]{0,80}\b(?:live|active)\b",
)
...
def _contains_external_write(text: str) -> bool:
    ...
    if _is_local_output_object_request(text):
        return False
    if _is_explicit_ad_campaign_read_only(text):
        return False
...
def _is_explicit_ad_campaign_read_only(text: str) -> bool:
    if not _contains_any_pattern(text, EXPLICIT_AD_CAMPAIGN_READ_ONLY_PATTERNS):
        return False
    opening = re.search(
        r"\b(?:analyze|analyse|audit|compare|evaluate|inspect|read|research|search|look up|review|summarize|summarise|explain|describe|define|what|which|how|find|list|show|give|send)\b",
        text,
    )
    if not opening:
        return False
    ...
    return not (
        _contains_any_pattern(remainder, AD_CAMPAIGN_PRODUCTION_ACTION_PATTERNS)
        or _contains_followup_external_write_signal(remainder)
    )


def _contains_ad_campaign_object(text: str) -> bool:
    return _contains_any_pattern(text, AD_CAMPAIGN_OBJECT_PATTERNS)


def _is_explicit_ad_campaign_public_search(text: str) -> bool:
    return _contains_any_pattern(text, EXPLICIT_AD_CAMPAIGN_PUBLIC_SEARCH_PATTERNS)
...
def _is_local_output_object_request(text: str) -> bool:
    ...
    if _contains_ad_campaign_object(normalized):
        return False
    return not (
        _contains_any_pattern(normalized, POSITIVE_CRM_DESTINATION_PATTERNS)
        or _contains_followup_external_write_signal(normalized)
        or (
            _contains_ad_campaign_object(normalized)
            and _contains_any_pattern(normalized, AD_CAMPAIGN_PRODUCTION_ACTION_PATTERNS)
        )
    )
...
def _is_safe_public_search_submission(text: str) -> bool:
    if not _contains_any_pattern(text, PUBLIC_SEARCH_SUBMISSION_PATTERNS):
        return False
    if _contains_ad_campaign_object(text):
        return _is_explicit_ad_campaign_read_only(text) or _is_explicit_ad_campaign_public_search(text)
    ...
    return not (
        ...
        or (
            _contains_ad_campaign_object(text)
            and _contains_any_pattern(remainder, AD_CAMPAIGN_PRODUCTION_ACTION_PATTERNS)
        )
    )
...
def _is_local_delivery_request(text: str) -> bool:
    if not _contains_any_pattern(text, LOCAL_DELIVERY_REQUEST_PATTERNS):
        return False
    if _contains_ad_campaign_object(text):
        return _is_explicit_ad_campaign_read_only(text)
    remainder = _local_delivery_remainder(text)
    return not (
        _contains_followup_external_write_signal(remainder)
        or (
            _contains_ad_campaign_object(text)
            and _contains_any_pattern(remainder, AD_CAMPAIGN_PRODUCTION_ACTION_PATTERNS)
        )
...
def _is_draft_only_text_workflow(text: str) -> bool:
    # Campaign drafting stays autonomous in Hermes, not in a browser provider run.
    # Any ad/campaign object passed to Super Browser must traverse approval.
    if _contains_ad_campaign_object(text):
        return False
