...
### Data / scraping lane (go get SPECIFIC data from SPECIFIC sites)

This is the scrape-lane, separate from the interactive-browser ladder above. Pick cheapest-capable-first, per the specialist profiles in `vault/super_browser/knowledge/providers/` (`apify.md`, `firecrawl.md`, `brightdata-dataset.md`):

- Named platform / niche site WITH a good maintained actor -> **Apify** (`apify_search_actors` -> `apify_actor_info` -> `apify_run_actor`, pilot before scale, $ cap enforced).
- Big-3 LinkedIn/Facebook/Google-Maps structured records at bulk -> **Bright Data datasets** (`dataset_scrape`).
- Generic public directory/listing page at volume -> **Firecrawl** (`industrial_scrape` provider=firecrawl).
- Interactive/logged-in/JS flow with NO actor -> a **browser** (Steel/Browserbase/browser_use, the ladder above).
