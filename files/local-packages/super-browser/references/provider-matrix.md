...
## Scrape / data lane (script-layer, not engine adapters)

These live in `scripts/` (lead_pipeline + apify_tools), not the engine's provider adapters, and serve "go get SPECIFIC data from SPECIFIC sites." Cheapest-capable-first; full profiles in `vault/super_browser/knowledge/providers/`.

| Lane | Status | Best for | Do not use when | Env |
| --- | --- | --- | --- | --- |
| Firecrawl | live | Fast clean markdown of public directory/listing pages at volume (~$0.003/pg) | Logins, hard anti-bot, a platform with a maintained actor/dataset | `FIRECRAWL_API_KEY` |
| Apify actors | evaluating | A NAMED platform/niche site with a maintained actor (~11k catalog) — the long tail | Big-3 bulk (use datasets), generic pages (use Firecrawl), unknown/untested actor at scale | `APIFY_TOKEN`; `APIFY_MAX_CHARGE_USD` (def 5), `APIFY_PILOT_ITEMS` (def 10) |
| Bright Data datasets | evaluating | Bulk STRUCTURED records from LinkedIn/Facebook/Google-Maps | Arbitrary directory sites | `BRIGHTDATA_API_KEY` |

⚠️ Apify runs cost real money (pay-per-result; one actor once burned $165 for nothing) — `apify_run_actor` enforces a hard `maxTotalChargeUsd` cap + a pilot; never auto-run an unknown actor at scale.
