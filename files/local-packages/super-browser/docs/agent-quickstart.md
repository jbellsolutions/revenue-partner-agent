...
## Weekly tool watch (maintainers)

```bash
python3 scripts/tool_watch.py --check          # verify every watched source still resolves
python3 scripts/tool_watch.py --run --dry-run  # detect + judge, print, write nothing
python3 scripts/tool_watch.py --run            # the weekly job: brief to Obsidian + Slack
```

Scheduled on the box by `super-browser-tool-watch.timer` (Mondays 08:00 ET). Watch list:
`vault/super_browser/watch/watchlist.yaml`. See `docs/tool-watch.md`.
